...
module LocationAudited
  ...
  class LocationAudit < ::ActiveRecord::Base
    ...
    def self.as_user(user)
      ...
      ::LocationAudited.store[:audited_user] = last_audited_user
    end
    ...
    def set_audit_user
      self.user ||= ::LocationAudited.store[:audited_user] # from .as_user
      self.user ||= ::LocationAudited.store[:current_user].try!(:call) # from Sweeper
      nil # prevent stopping callback chains
    end

    def set_request_uuid
      self.request_uuid ||= ::LocationAudited.store[:current_request_uuid]
      self.request_uuid ||= SecureRandom.uuid
    end

    def set_remote_address
      self.remote_address ||= ::LocationAudited.store[:current_remote_address]
    end
  end
end
