module LocationAudited
  ...
  #
  # See <tt>LocationAudited::LocationAuditor::ClassMethods#location_audited</tt>
  # for configuration options
  module LocationAuditor #:nodoc:
    ...
    module ClassMethods
      ...
      #   By default, LocationAudited will audit all but these fields:
      #
      #     [self.primary_key, inheritance_column, 'lock_version', 'created_at', 'updated_at']
      #   You can add to those by passing one or an array of fields to skip.
      #
      #     class User < ActiveRecord::Base
      #       location_audited except: :password
      #     end
      #
      ...
      #
      #     class User < ActiveRecord::Base
      #       location_audited redacted: :password, redaction_value: SecureRandom.uuid
      #     end
      #
      ...
      #     end
      #
      def location_audited(options = {})
        # don't allow multiple calls
        return if included_modules.include?(LocationAudited::LocationAuditor::AuditedInstanceMethods)

        extend LocationAudited::LocationAuditor::AuditedClassMethods
        include LocationAudited::LocationAuditor::AuditedInstanceMethods
        ...
        has_many :audits, -> { order(version: :asc) }, as: :auditable, class_name: LocationAudited.audit_class.name, inverse_of: :auditable
        LocationAudited.audit_class.audited_class_names << to_s
      ...
      def has_associated_audits
        has_many :associated_audits, as: :associated, class_name: LocationAudited.audit_class.name
      end
    end
    ...
    module AuditedInstanceMethods
      ...
      def revision(version)
        if version == :previous || self.audits.last.version >= version
          revision_with LocationAudited.audit_class.reconstruct_attributes(audits_to(version))
        end
      end
      ...
      def revision_at(date_or_time)
        ...
        revision_with LocationAudited.audit_class.reconstruct_attributes(audits) unless audits.empty?
      end

      # List of attributes that are location_audited.
      ...
      def own_and_associated_audits
        LocationAudited.audit_class.unscoped
      ...
      def revision_with(attributes)
        dup.tap do |revision|
          ...
          LocationAudited.audit_class.assign_revision_attributes(revision, attributes)
    ...
    module AuditedClassMethods
      # Returns an array of columns that are location_audited. See non_audited_columns
      def audited_columns
        @audited_columns ||= column_names - non_audited_columns
      end

      # We have to calculate this here since column_names may not be available when `location_audited` is called
      def non_audited_columns
        @non_audited_columns ||= calculate_non_audited_columns
      end

      def non_audited_columns=(columns)
        @audited_columns = nil # reset cached location_audited columns on assignment
      ...
      def audit_as(user, &block)
        LocationAudited.audit_class.as_user(user, &block)
      end

      def auditing_enabled
        LocationAudited.store.fetch("#{table_name}_auditing_enabled", true) && LocationAudited.auditing_enabled
      end

      def auditing_enabled=(val)
        LocationAudited.store["#{table_name}_auditing_enabled"] = val
      end

      def default_ignored_attributes
        [primary_key, inheritance_column] | LocationAudited.ignored_attributes
      end
      ...
      def normalize_audited_options
        ...
        max_audits = audited_options[:max_audits] || LocationAudited.max_audits
