module LocationAudited
  class LocationSweeper
    ...
    delegate :store, to: ::LocationAudited
    ...
    def current_user
      lambda { controller.send(LocationAudited.current_user_method) if controller.respond_to?(LocationAudited.current_user_method, true) }
    end
